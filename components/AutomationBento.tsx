...
const TypewriterDisplay = memo(function TypewriterDisplay() {
  ...
  return (
    <div className="flex items-center gap-2 font-mono text-xs text-zinc-800 dark:text-zinc-300 bg-zinc-50 dark:bg-zinc-800/60 rounded-xl px-3 py-2 min-h-[2.5rem]">
    ...
    <div className="overflow-hidden -mx-2">
      <div ref={containerRef} className="flex gap-2 w-max">
        {doubled.map((item, i) => (
          <span
            key={i}
            className="shrink-0 text-[10px] font-mono text-zinc-800 dark:text-zinc-300 border border-zinc-200 dark:border-zinc-700 rounded-full px-2.5 py-1 bg-white dark:bg-zinc-900"
          >
            {item}
          </span>
        ))}
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <p className="text-sm text-zinc-800 leading-relaxed mt-1">{a.description}</p>
      </div>

      {/* Metric */}
      <div className="flex items-end gap-1">
        <span className="text-3xl font-mono font-semibold text-zinc-900 dark:text-zinc-100">{a.metric}</span>
        <span className="text-xs text-zinc-800 pb-1">{a.metricLabel}</span>
      </div>
      ...
      <div className="min-h-[2.5rem]">
        ...
        {a.animation === "highlight" && (
          <div className="flex items-center gap-2 text-xs text-zinc-800 font-mono">
      ...
      <div className="flex flex-wrap gap-1.5 mt-auto">
        {a.tags.map((t) => (
          <span key={t} className="text-[10px] font-mono bg-zinc-100 dark:bg-zinc-800 text-zinc-800 dark:text-zinc-300 rounded-full px-2.5 py-0.5">
            {t}
          </span>
        ))}
      </div>
      ...
      <button
        ...
        className="flex items-center gap-1.5 text-[11px] font-mono text-zinc-800 hover:text-accent transition-colors duration-200 w-fit"
      >
      ...
      <AnimatePresence initial={false}>
        {expanded && (
          ...
          >
            <p className="text-xs text-zinc-800 leading-relaxed border-t border-white/40 pt-3">
              {a.how}
            </p>
          </motion.div>
        )}
  ...
  return (
    <section id="automations" className="relative py-24 px-6 md:px-10">
      ...
      >
        ...
        <a
          href="#contact"
          className="inline-flex items-center gap-2 text-sm text-zinc-800 hover:text-zinc-900 transition-colors duration-200 group self-start md:self-auto shrink-0"
        >
      ...
      <div className="mt-10 flex justify-center">
        <a
          href="#contact"
          className="group inline-flex items-center gap-2 text-sm font-medium text-zinc-800 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors"
        >
